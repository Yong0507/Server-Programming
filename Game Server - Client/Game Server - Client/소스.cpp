...
//    DWORD receiveBytes;
    ...
    case WM_KEYDOWN:
        ...
        {
        case VK_LEFT:
            socketInfo->test.num = 1;
            socketInfo->dataBuffer.len = sizeof(socketInfo->test);
            socketInfo->dataBuffer.buf = (char*)&socketInfo->test;
            WSASend(s_socket, &socketInfo->dataBuffer, 1, &num_sent, 0, NULL, NULL);
            break;
        case VK_RIGHT:
            socketInfo->test.num = 2;
            socketInfo->dataBuffer.len = sizeof(socketInfo->test);
            socketInfo->dataBuffer.buf = (char*)&socketInfo->test;
            WSASend(s_socket, &socketInfo->dataBuffer, 1, &num_sent, 0, NULL, NULL);
            break;
        case VK_DOWN:
            socketInfo->test.num = 3;
            socketInfo->dataBuffer.len = sizeof(socketInfo->test);
            socketInfo->dataBuffer.buf = (char*)&socketInfo->test;
            WSASend(s_socket, &socketInfo->dataBuffer, 1, &num_sent, 0, NULL, NULL);
            break;
        case VK_UP:
            socketInfo->test.num = 4;
            socketInfo->dataBuffer.len = sizeof(socketInfo->test);
            socketInfo->dataBuffer.buf = (char*)&socketInfo->test;
            WSASend(s_socket, &socketInfo->dataBuffer , 1, &num_sent, 0, NULL, NULL);
            break;
        ...
        WSARecv(s_socket, &socketInfo->dataBuffer, 1, &num_recv, &flag, NULL, NULL);
